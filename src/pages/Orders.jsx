import React from 'react'

const Orders = () => {
  return (
    <>
    <div class="top-section container">
        <div class="sect-1">
          <h5>Orders</h5>
          <small>Current Orders</small>
        </div>
      </div>
      <div class="container mt-3">
        <table class="table table-hover">
        <thead>
          <tr>
            <th scope="col">#</th>
            <th scope="col">Order</th>
            <th scope="col">Code</th>
            <th scope="col">Customer</th>
            <th scope="col">Date</th>
            <th scope="col">Status</th>
            <th scope="col">Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <th scope="row">1</th>
            <td>Butter Cake</td>
            <td>001</td>
            <td>Juin Juillet</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
              <span class="btn"> <a href="/orderinfo"><i class='bx bx-info-circle text-black'></i> </a></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">2</th>
            <td>Sponge Cake</td>
            <td>002</td>
            <td>Aston Mark</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">3</th>
            <td>Angel Food Cake</td>
            <td>003</td>
            <td>Lucas Graham</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-check-double" style={{color:'green'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">4</th>
            <td>Chiffon Cake</td>
            <td>004</td>
            <td>James Wright </td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">5</th>
            <td>Bundt Cake</td>
            <td>005</td>
            <td>Evans Kingston</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">6</th>
            <td>Fruit Cake</td>
            <td>006</td>
            <td>Quinton McNaughty</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">7</th>
            <td>Carrot Cake</td>
            <td>007</td>
            <td>Silloute Sean</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">8</th>
            <td>Red Velvet Cake</td>
            <td>008</td>
            <td>Angel Reese</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">9</th>
            <td>Opera Cake</td>
            <td>009</td>
            <td>Canon Cain</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">10</th>
            <td>Black Forest Cake</td>
            <td>010</td>
            <td>Micheal Myers</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">11</th>
            <td>Gateau St.Honore</td>
            <td>011</td>
            <td>Grace Simm</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
          <tr>
            <th scope="row">12</th>
            <td>Flour Chocolate Cake</td>
            <td>012</td>
            <td>Martin Garrix</td>
            <td>Cakes</td>
              <td>
              <div class="top-section">
                <i class="bx bx-bowl-hot" style={{color:'red'}}></i><br/>
              </div>
            </td>
            <td>
              <div class="top-section">
                <span class="btn"><i class='bx bx-info-circle'></i></span>
              </div>
            </td>
          </tr>
        </tbody>
      </table>
      </div>
    </>
  )
}

export default Orders
